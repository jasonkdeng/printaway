...
export function GoogleSignInButton({ className, label, callbackUrl = "/cart" }: GoogleSignInButtonProps) {
  ...
  return (
    <button aria-busy={isStarting || undefined} className={className} disabled={isStarting} onClick={() => void beginGoogleSignIn()} type="button">
      ...
      <span>{isStarting ? "Opening Google" : label}</span>
    </button>
  );
}
