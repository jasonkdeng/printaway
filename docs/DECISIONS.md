...
- Initial catalog fixtures are Monitor Riser (PLA, 100 g, 350 x 250 x 120 mm, CAD $12.00), Desk Tray (PLA, 50 g, 120 x 180 x 100 mm, CAD $3.00), Coat Hanger (ABS, 15 g, 400 x 10 x 150 mm, CAD $2.00), and Keycap Fidget (PLA, 10 g, 100 x 30 x 30 mm, CAD $3.00). All are currently sold out. Each supports Default or Glossy finish; Glossy surcharges are $2.00, $1.00, $0.50, and $0.20 respectively. The approved Bambu Lab colour basis is white, black, gray, dark green, tan, latte brown, and red. Product summaries and limitations are approved; media remains placeholder-only until Jason Deng supplies owned photographs or renders. Square catalog prices and modifiers must be updated and verified against these approved amounts before checkout is enabled.
...
- Each currently configured Square variation ID represents shared sellable inventory for its matching Shop product's listed finish and colour selections. Google-backed account-cart persistence is implemented in code and awaits the account-cart migration plus live OAuth verification. Square checkout remains unimplemented.
...
- Live Google OAuth and account-cart persistence verification.
- Approved production media and provenance manifests.

## Delivery note

The Stage 2 and Stage 4 Supabase migrations were applied to project `kczuhxsclbephazisqoa` on 2026-07-20. The private `reference-uploads` bucket is present and non-public. The follow-up rate-limit privilege migration restricts the SECURITY DEFINER function to the server role. The account-cart migration remains ready for one-time SQL Editor application and must be recorded here after Jason applies it.
